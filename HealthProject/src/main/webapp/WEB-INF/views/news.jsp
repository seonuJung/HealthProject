...
$(function(){
	$("#viewCount").change(function() {
		var value = $(this).val();
		sendPost('news', {"p":${pv.currentPage},"s":value,"b":${pv.blockSize}})
	});
});
</script>
</head>
<body>
	<table>
		<tr>
			<td colspan="5" align="center" class="title">공지사항</td>
		</tr>
		<tr>
				...
				<c:forEach var="vo" items="${pv.list }" varStatus="vs">
					<tr align="center"  onmouseover="this.style.backgroundColor='9ce7ef'" onmouseout="this.style.backgroundColor='white'">
						...
						<td align="left"  onmouseover="this.style.fontWeight='bold'" onmouseout="this.style.fontWeight='normal'">
								<span style="cursor: pointer" onclick="sendPost('${pageContext.request.contextPath }/newsview',{'notice_idx': ${vo.notice_idx }});">
								
								<c:if test="${fn:length(vo.sys_title) lt 30 }">
									<c:out value="${vo.sys_title }"/>	
								</c:if>
								<c:if test="${fn:length(vo.sys_title) ge 30 }">
									<span title="${vo.sys_title }">
										<c:out value="${fn:substring(vo.sys_title,0,30) }"/> ...	
									</span>
								</c:if>
								</span>
						</td>
								<td>
									<c:out value="${vo.sys_name }"/>
								</td>
								<td>
									<fmt:formatDate value="${vo.sys_date }" pattern="yy-MM-dd"/>
								</td>
					</tr>
				</c:forEach>
				<tr>
		...
		<tr>
			<td colspan="5" class="info">
				...
				<button onclick="location.href='newsinsert'">글쓰기</button>
				</c:if>
